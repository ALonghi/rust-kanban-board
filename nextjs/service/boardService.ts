...
import {ApiResponse, CreateBoardColumnResponse, CreateBoardRequest} from "../model/dto";
...
export default class BoardService {
    ...
    static async createBoardColumn(request: CreateBoardRequest, boardId: IBoard['id']): Promise<CreateBoardColumnResponse> {
        try {
            Logger.info(`Adding board column ${JSON.stringify(request)} for board ${boardId}`);
            const response = (await axios
            ...
            if (!response.success || !response.data)
                throw new Error(response.error_message);
            Logger.info(
                `Added board column with id ${response.data?.column?.id}`
            );
            return response.data;
        } catch (e) {
            Logger.error(
                `Error in creating board column (${JSON.stringify(request)}) for board ${boardId}: ${e.message || e}`
            );
            return Promise.reject(e);
        }
    }
